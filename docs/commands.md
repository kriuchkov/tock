...
| `stop`, `s` | Stop the current activity | `-t` end time (HH:MM) | `tock stop -t 17:00` |
...
| `continue`, `c` | Continue a previous activity | `-d` override description<br>`-p` override project<br>`-t` start time | `tock continue 1` |
...
| `last`, `lt` | List recent unique activities | `-n` number of activities (default 10) | `tock last -n 20` |
| `list`, `ls` | Interactive calendar view (TUI) | - | `tock list` |
